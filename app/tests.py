...
def update_moshaver(request, information_id):
    ...
    if request.method == 'POST':
        formsy = information_m_f(request.POST, instance=information)
        if formsy.is_valid():
        ...
        formsy = information_m_f(instance=information)
    return render(request, 'sabt_moshaver.html', {'form': formsy})
################################################پنل شخصی مشاور
...
def update_img(request, id):
    tabel = get_object_or_404(image, id=id)
    if request.method == 'POST':
        form = image_f(request.POST, request.FILES, instance=tabel)
        if form.is_valid():
            form.save()
            return HttpResponseRedirect('/list_img')
    else:
...
def update_padcast(request, id):
    ...
    return render(request, 'sabt_padcast.html', {'form': form})
###################################################################تست ها 
def test_d(request):
    user_st = userauth().state_and_login(request)
    print(user_st)
...
def delet_test(request, id):
    ...
    if request.method == 'POST':
        tabel.delete()
        return HttpResponseRedirect('/list_test')
    return render(request, 'delet_test.html', {"tabel": tabel})
...
def update_test(request, id):
    ...
    return render(request, 'sabt_test.html', {'form': form})
...
def update_vidio(request, id):
    tabel = get_object_or_404(vidio, id=id)
    if request.method == 'POST':
        form = vidio_f(request.POST, request.FILES, instance=tabel)
        if form.is_valid():
            form.save()
            return HttpResponseRedirect('/list_vidio')
    else:
